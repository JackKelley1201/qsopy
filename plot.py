...
def plot_object():
    ...
    theoretical_locations = trough_identify.theoretical_doublets(data, matched_mgii)
    ...
    for z in matched_mgii:
        color = np.random.random(3)
        ax.vlines(data['Observed Wavelength'].loc[matched_mgii[z][0]], 0, data['Flux'].max(), color=color,
                  linewidth=0.6, label=np.round(z, decimals=3))
        # plot red
        ax.vlines(data['Observed Wavelength'].loc[matched_mgii[z][1]], 0, data['Flux'].max(), color=color,
                  linewidth=0.6)

        for theoretical_doublet in theoretical_locations[z]:
            ax.vlines(data['Observed Wavelength'].loc[theoretical_doublet[0]], 0, data['Flux'].max(), color=color,
                      linewidth=0.6, label=(str(np.round(z, decimals=3)) + " " + theoretical_doublet[2]))
            # plot red
            ax.vlines(data['Observed Wavelength'].loc[theoretical_doublet[1]], 0, data['Flux'].max(), color=color,
                      linewidth=0.6)

            ax.annotate(str(np.round(z, decimals=3)) + " " + theoretical_doublet[2] + '*',
                        xy=(data['Observed Wavelength'].loc[theoretical_doublet[1]], 25),
                        xytext=(data['Observed Wavelength'].loc[theoretical_doublet[1]] + 5, 25))
